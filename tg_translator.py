...
async def _run_generation(query_or_message, uid: int, extra_details: str):
    ...
    # Extract only generation_prompt — that's all NB Pro Edit needs
    extracted_prompt = final_prompt  # fallback
    try:
        clean = final_prompt.strip()
        if clean.startswith("```"):
            clean = clean.split("```")[1]
            if clean.startswith("json"):
                clean = clean[4:]
        parsed = json.loads(clean)
        if "generation_prompt" in parsed:
            extracted_prompt = parsed["generation_prompt"]
        elif "generation_parameters" in parsed:
            prompts = parsed["generation_parameters"].get("prompts", [])
            if prompts:
                extracted_prompt = prompts[0]
    except Exception:
        pass
    ...
    full_text = header + extracted_prompt + footer
